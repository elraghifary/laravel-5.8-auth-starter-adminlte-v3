        ...
        foreach ($roles as $key => $role) {
            ...
            $output['data'][$i][] = '
                <form action="' . $link_delete . '" method="post" class="form-delete">
                    <a href="' . $link_permission . '" class="btn btn-info btn-xs" data-popup="tooltip" title="Permission Role"><i class="fas fa-file-alt"></i></a>
                    <input type="hidden" name="_token" value="' . csrf_token() . '">
                    <input type="hidden" name="_method" value="DELETE">
                    <button class="btn btn-danger btn-xs" data-popup="tooltip" title="Delete Role"><i class="fas fa-trash"></i></button>
                </form>';
            $i++;
        }
